module miko::marketplace {
	...
	struct Listed has drop, store { id: u64, seller: address, amount: u64, price: u64 }
	struct Purchased has drop, store { id: u64, buyer: address, amount: u64, total_paid: u64, fee: u64 }
	struct Delisted has drop, store { id: u64 }
	...
	public entry fun init(admin: &signer, fee_bps: u64) {
		...
		move_to(admin, Events {
				...
				purchased: account::new_event_handle<Purchased>(admin),
	...
	public entry fun buy(buyer: &signer, listing_id: u64, amount: u64) acquires Registry, Events, Escrows {
		assert!(amount > 0, E_ZERO);
		let registry = borrow_global_mut<Registry>(@admin);
		// copy top-level registry fields before taking a mutable borrow into a listing
		let fee_bps = registry.fee_bps;
		let fee_treasury = registry.fee_treasury;

		// now borrow the listing (this creates a mutable borrow into registry)
		let (l_ref, idx) = borrow_listing_mut(registry, listing_id);
		assert!(l_ref.remaining >= amount, E_NOT_ENOUGH);

		// copy all needed listing fields into locals so we don't rely on `l_ref` after
		let seller = l_ref.seller;
		let unit_price = l_ref.unit_price;

		// operate on escrow (separate resource) to extract coins
		let esc = borrow_global_mut<Escrows>(@admin);
		let pool = table::remove(&mut esc.inner, listing_id);
		let to_transfer = coin::extract(&mut pool, amount);

		let fee = amount * fee_bps / 10_000;
		if (fee > 0) {
			let fee_part = coin::extract(&mut to_transfer, fee);
			coin::deposit(fee_treasury, fee_part);
		};
		...
		if (new_remaining > 0) {
			// put remaining coins back into escrow and update the in-place listing
			table::add(&mut esc.inner, listing_id, pool);
			l_ref.remaining = new_remaining;
		} else {
			// no remaining: return leftover pool to seller or destroy
			if (coin::value(&pool) > 0) {
				coin::deposit(seller, pool);
			} else { coin::destroy_zero<miko::cct::CCT>(pool); };
			// remove the listing from registry (we've already copied needed fields)
			remove_listing(registry, idx);
		};

		let ev = borrow_global_mut<Events>(@admin);
		event::emit_event(&mut ev.purchased, Purchased { id: listing_id, buyer: signer::address_of(buyer), amount, total_paid: amount * unit_price, fee });
	}
	...
	public fun get_listing(id: u64): Option<Listing> acquires Registry {
		let r = borrow_global<Registry>(@admin);
		let len = vector::length(&r.list);
		let i = 0;
		while (i < len) {
